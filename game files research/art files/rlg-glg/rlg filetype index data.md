...
For instance, in mario.rlg I want to find the index offset of the first mesh, so I go to the mesh data section (identifier 0x8004)
...
*This could actually be 8bit integers for some files, but I haven't really explored that. 
 ...
 Afaik, if it's 0x0000 it's 16bit, if it's 0x8000 it's 8bit, though I haven't 8bit indices being used yet.
...
For those who don't know, indices reference vertices. A triangle face is made up of three vertices, which are referenced indirectly by the indices of the face.

To find which verts a face is associated to:
- For each of the face's index:
- Find the vertex that is identified by that value
- For instance, if my index is 0, I look for to the 1st vertex of this meshe's vertex section. If it's 123 I go to the 124th vertex of this meshe's vertex section.