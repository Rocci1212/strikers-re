...
# Useful register: r8 input bitfield
...
MAKE_TEAM_DISAPPEAR:    
    ...
    lis r5, 0               # r5 = float 0      X coord we're sending the captain to
    lis r16, 0x8056			# Load address for Player objects in r16
    ori r16, r16, 0xA740
    # PAL rev1: 8056A740
    # PAL rev2:
    # NTSC-U:
...
SET_ITEM_QUANTITY_NOT_ZERO:
    ...
    bne DOUBLE_INPUT_CHECK
    li r10, 1

DOUBLE_INPUT_CHECK:
    lbz r16, 0x1552 (r12)			# Load a value that tells the code if a button was pressed in the last frame
    cmpwi r16, 0
    bne- DOUBLE_INPUT_DETECT
...
CHECK_INPUT_UP:
	...
	bge DOUBLE_INPUT_DETECT
	addi r10, r10, 1            # increment quantity

DOUBLE_INPUT_DETECT:
    li r16, 0
    andi. r0, r8, 0x000f	# Do a bitwise AND between r8 and 0x000f. Each of the 4 d-pad buttons sets one of the last bits high and we want the code to take the branch if none is pressed.
    beq+ STORE_ITEMS
	li r16, 1			# D-PAD HAS BEEN PRESSED ON THIS FRAME, therefore player's input will be ignored starting from the next frame until the player releases the button.

STORE_ITEMS:
    stb r5, 0x1550	(r12)		# Store the item you picked into 0x80001550
    stb r10, 0x1551	(r12)		# Store item quantity into 0x80001551
    stb r16, 0x1552 (r12)		# Update the value in 0x80001552
    li r11, 0xFFFFffff	        # r11 = -1
    lis r16, 0x806e
    ...
    # NTSC-J:
    # NTSC-K:
    beq cr7, GIVE_ITEM_TO_AWAY # Check if training mode is on for home or if it's for away
...
GIVE_ITEM_TO_AWAY:
    stw r5, 0xbf4 (r16)			# Store away's new item
    stw r10, 0xbf8 (r16)		# Store away's new item quantity
    stw r11, 0x8c (r16)			# Remove home's items
    b END
